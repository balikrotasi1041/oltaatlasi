...
for (const route of meralar) {
  ...
  if (!hasCoordinates) {
    ...
    const evidenceBackedGeneralC = route.confidence === "C"
      && route.locationPrecision === "Genel bölge"
      && route.navigationVerified === false
      && route.confidenceProfile?.identity.level === "strong"
      && route.confidenceProfile?.access.level !== "unverified";
    if (route.confidence !== "D" && !evidenceBackedGeneralC) errors.push(`${route.slug}: çözülmemiş koordinat yalnız D veya açıkça Genel bölge olarak etiketli, rota kimliği/erişimi kaynaklı C kayıtta yayımlanabilir.`);
...
console.log(`Bakım taraması: ${meralar.length} rota; confidence A/B/C/D = ${confidence.A}/${confidence.B}/${confidence.C}/${confidence.D}; ${unresolvedCoordinateCount} çözülmemiş/genel pin; ${errors.length} hata, ${warnings.length} uyarı.`);
