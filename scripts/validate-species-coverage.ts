...
import { probableSpeciesByRoute, speciesProfileGap, unsourcedProbableOccurrences } from "../src/data/species-occurrence";
import { speciesRegulations, provinceSeasonRules, nationalAmateurSource, nationalAmateurAmendment2025 } from "../src/data/species-regulations";
...
const missingRegulations=activeSpecies.filter((name)=>!speciesRegulations[name]);
...
for(const item of unsourcedProbableOccurrences())errors.push(`Kaynağı eksik muhtemel tür: ${item.routeSlug} / ${item.species}`);
...
console.log(`Tür kapsamı: ${activeSpecies.length} aktif avlak türü, ${baliklar.length} balık profili, ${Object.keys(speciesRegulations).length} doğrulanmış mevzuat profili, ${probableEntries.length} kaynaklı muhtemel eşleşme.`);
