...
type NationalSeed = {
  ...
  province: string;
  zone: string;
  waterType: Mera["waterType"];
  sourceKind: "inventory" | "supplemental";
};

export const NATIONAL_ROUTE_PROVINCE_COUNT = 81;
export const NATIONAL_ROUTE_COUNT_PER_PROVINCE = 5;
export const NATIONAL_ROUTE_EXPECTED_COUNT = NATIONAL_ROUTE_PROVINCE_COUNT * NATIONAL_ROUTE_COUNT_PER_PROVINCE;
...
const waterTypeFor = (name: string): Mera["waterType"] => {
  const normalized = name.toLocaleLowerCase("tr-TR");
  if (normalized.includes("sahili")) return "Deniz";
  if (normalized.includes("baraj")) return "Baraj";
  if (normalized.includes("gölet")) return "Gölet";
  if (/(nehri|çayı|deresi|ırmağı|suyu|iyidere|büyükdere|değirmendere)/.test(normalized)) return "Akarsu";
  return "Göl";
};

const nationalSeeds: NationalSeed[] = rawRoutes
  .trim()
  .split("\n")
  .flatMap((line) => {
    const [province, zone, namesText] = line.split("|");
    if (!province || !zone || !namesText) throw new Error(`Ulusal rota satırı okunamadı: ${line}`);
    return namesText.split(";").map((name) => {
      const waterType = waterTypeFor(name);
      return {
        ...
        zone,
        waterType,
        sourceKind: waterType === "Baraj" ? "inventory" : "supplemental",
      };
    });
  });

if (nationalSeeds.length !== NATIONAL_ROUTE_EXPECTED_COUNT) {
  throw new Error(`Ulusal rota sayısı ${NATIONAL_ROUTE_EXPECTED_COUNT} yerine ${nationalSeeds.length}.`);
}

const dsiSource: SourceLink = {
  label: "Devlet Su İşleri – Resmî su kaynakları istatistikleri",
  url: "https://dsi.gov.tr/Sayfa/Detay/2186",
  note: "Türkiye su yapıları ve su kaynakları için resmî kurumsal başvuru kaynağıdır; belirli bir kıyıda av izni verildiği anlamına gelmez.",
};

const inventorySource: SourceLink = {
  label: "Türkiye Baraj Envanteri – DSİ tabanlı açık veri derlemesi",
  url: "https://turkiyebarajlar.com/envanter",
  note: "Baraj adları ve il gruplaması için kullanılan açık envanterdir; bağlayıcı ve güncel saha teyidi için DSİ ve yerel kurum kaynakları ayrıca incelenmelidir.",
};

const waterAtlasSource: SourceLink = {
  label: "Tarım ve Orman Bakanlığı – Türkiye Su Atlası",
  url: "https://www.tarimorman.gov.tr/SYGM/Sayfalar/GormeEngellilerDetay.aspx?Liste=Haber&OgeId=1349",
  note: "Türkiye'nin yerüstü su kaynaklarını havza, coğrafya ve yönetim boyutlarıyla ele alan resmî başvuru kaynağıdır.",
};

const amateurRulesSource: SourceLink = {
  ...
  note: "1 Eylül 2024–31 Ağustos 2028 dönemindeki genel amatör avcılık kurallarıdır; yerel ve suya özel yasaklar ayrıca kontrol edilmelidir.",
};

const amendmentSource: SourceLink = {
  label: "6/2 Tebliğ değişikliği – Tarım ve Orman Bakanlığı",
  ...
  note: "2025 yılında yayımlanan değişiklikleri gösterir; güncel resmî metin av öncesinde yeniden kontrol edilmelidir.",
};

const shoreProfileFor = (seed: NationalSeed) => {
  if (seed.waterType === "Deniz") return "Deniz kıyısı; plaj, liman, yüzme alanı, kıyı tesisi ve özel kullanım sınırları kısa mesafede değişebilir.";
  if (seed.waterType === "Akarsu") return "Akarsu koridoru; debi, kıyı eğimi, taşkın riski, özel mülkiyet ve ulaşılabilir kıyı bölümleri mevsime göre değişebilir.";
  if (seed.waterType === "Baraj") return "Baraj gölü çevresi; işletme, içme suyu, enerji, sulama, güvenlik ve koruma statüleri kıyı kullanımını sınırlayabilir.";
  if (seed.waterType === "Gölet") return "Gölet çevresi; su seviyesi, işletme amacı, kıyı zemini ve yerel kullanım kararları ayrıca araştırılmalıdır.";
  return "Göl çevresi; koruma statüsü, sulak alan sınırı, kıyı erişimi, su seviyesi ve yerel kullanım kuralları ayrıca araştırılmalıdır.";
};

const makeNationalRoute = (seed: NationalSeed): Mera => {
  const mapSearch = `https://www.openstreetmap.org/search?query=${encodeURIComponent(`${seed.name}, ${seed.province}, Türkiye`)}`;
  const placeSource: SourceLink = {
    label: `OpenStreetMap ad ve konum araması – ${seed.name}`,
    url: mapSearch,
    note: "Su varlığının açık haritadaki ad ve genel konum eşleşmesini kontrol etmek içindir; yol tarifi, kamusal erişim veya avlanma izni değildir.",
  };
  const verification = seed.sourceKind === "inventory"
    ? "Adı ve il ilişkisi DSİ tabanlı açık baraj envanteri, resmî su kaynakları başvuru sayfaları ve açık harita aramasıyla kayıt düzeyinde kontrol edildi; kıyı erişimi ve avlanma izni doğrulanmadı."
    : "Adı ve il ilişkisi resmî su atlası/DSİ başvuru kaynakları ile açık harita araması üzerinden kayıt düzeyinde kontrol edildi; kıyı erişimi ve avlanma izni doğrulanmadı.";
  ...
  return {
    ...
    zone: seed.zone,
    waterType: seed.waterType,
    region: seed.zone,
    summary: `${seed.name}, ${seed.province} için açık kaynaklarda su varlığı olarak eşleştirilmiş ön değerlendirme rotasıdır; kamusal erişim ve avlanma uygunluğu yola çıkmadan önce ayrıca doğrulanmalıdır.`,
    fish: [],
    methods: [],
    baits: [],
    ...
    amenities: [],
    cautions: [
      "Kesin kamusal kıyı erişim noktası yayımlanmamıştır.",
      "Özel mülkiyet, içme suyu, koruma alanı, güvenlik veya işletme sınırı bulunabilir.",
      "Avlanma yasağı, dönem, tür, boy ve adet kuralları güncel resmî kaynaklardan kontrol edilmelidir.",
      "Hava, su seviyesi, akıntı, debi, zemin ve yol koşulları yerinde yeniden değerlendirilmelidir.",
    ],
    locationPrecision: "Yayımlanmadı",
    verification,
    updatedAt: "2026-07-30",
    publishedAt: "2026-07-30",
    confidence: "D",
    image: `/images/meralar/ulusal/${seed.slug}.svg`,
    socialImage: `/images/meralar/ulusal/${seed.slug}.svg`,
    navigationNote: "Yanlış hassasiyet ve izinsiz erişim riski oluşturmamak için kesin rota noktası verilmemiştir. Açık harita araması yalnızca su varlığının genel çevresini araştırmak için kullanılmalıdır.",
    shoreProfile: shoreProfileFor(seed),
    transport: "Yol, park, son yaya yaklaşımı ve kamusal kıyı erişimi doğrulanmış değildir. İlgili belediye, kaymakamlık, DSİ birimi, su idaresi, milli park veya alan işletmesinden güncel bilgi alınmalıdır.",
    crowdNote: "Kullanım yoğunluğu, tarım ve tesis faaliyetleri, yüzme sezonu, av baskısı ve yerel etkinlikler nedeniyle değişebilir; güvenli atış koridoru bulunmayan yerde olta açılmamalıdır.",
    longIntro: [
      `${seed.name}, ${seed.province} ili için ulusal toplu tarama kapsamında oluşturulmuş bir ön değerlendirme rota dosyasıdır. Su varlığının adı ve il eşleşmesi açık kaynak kayıtlarında kontrol edilmiş, ancak belirli bir kıyı noktasının amatör balıkçılığa açık olduğu sonucu çıkarılmamıştır.`,
      "Bu kaydın güven seviyesi D'dir. Fiziki erişim, coğrafi kısıt, özel mülkiyet, koruma veya güvenlik statüsü, güncel su seviyesi ve hukuki avlanma uygunluğu kullanıcı tarafından resmî kaynaklardan ve yerinde ayrıca doğrulanmalıdır.",
    ],
    planningNotes: [
      "Yola çıkmadan önce su varlığının bağlı olduğu kurum, il/ilçe tarım müdürlüğü ve yerel yönetim duyuruları araştırılmalıdır.",
      "İçme suyu havzası, baraj güvenlik bölgesi, milli park, sulak alan, üretim tesisi, sınır veya askerî güvenlik alanı ihtimali ayrıca kontrol edilmelidir.",
      "Tabela, bariyer, görevli talimatı ve yerel idari kararlar bu sayfadaki açık kaynak özetinden üstündür; belirsizlik varsa alana girilmemelidir.",
    ],
    seasonalNotes: [
      "Bu ön değerlendirme kaydında rota bazında doğrulanmış balık türü listesi yayımlanmamıştır; bölgesel tür bilgisinden belirli noktada av sonucu çıkarılmamalıdır.",
      "Tür, dönem, boy, adet, takım ve alıkoyma kuralları güncel 6/2 Tebliğ, değişiklikleri ve yerel kararlarla birlikte kontrol edilmelidir.",
    ],
    sources: [
      placeSource,
      seed.sourceKind === "inventory" ? inventorySource : waterAtlasSource,
      dsiSource,
      amateurRulesSource,
      amendmentSource,
    ],
...
export const ulusalMeralar: Mera[] = nationalSeeds.map(makeNationalRoute);
