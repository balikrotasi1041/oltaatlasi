...
const common=(slug:string,name:string,district:string,province:string,zone:string,lat:number,lng:number,image:string,sources:ResearchSource[],mapSource:ResearchSource,summary:string,identity:string,access:string,waterType="Gölet",region="İç Anadolu"):EnrichedMera=>({
