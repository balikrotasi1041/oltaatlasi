...
const overridden=(slug:string)=>beykozSlugs.has(slug)||kocaeliSlugs.has(slug)||istanbulSlugs.has(slug)||gunlukIyilestirmeSlugs20260801.has(slug)||gunlukIyilestirmeSlugs20260802.has(slug)||kaliteIyilestirmeSlugs20260803.has(slug)||gunlukKaliteSlugs20260803.has(slug)||superseded20260807(slug);
export const meralar:EnrichedMera[]=[
  ...
  ...beykozMeralar.filter(m=>!retiredRouteSlugs20260803.has(m.slug)&&!istanbulSlugs.has(m.slug)&&!superseded20260807(m.slug)).filter(m=>!gunlukIyilestirmeSlugs20260801.has(m.slug)&&!gunlukIyilestirmeSlugs20260802.has(m.slug)).map(baseDefaults),
  ...kocaeliIyilestirmeler.filter(m=>!retiredRouteSlugs20260803.has(m.slug)&&!gunlukIyilestirmeSlugs20260801.has(m.slug)&&!gunlukIyilestirmeSlugs20260802.has(m.slug)&&!superseded20260807(m.slug)).map(m=>withResearch(m,kocaeliMeta[m.slug])),
  ...istanbulIyilestirmeler.filter(m=>!retiredRouteSlugs20260803.has(m.slug)&&!gunlukIyilestirmeSlugs20260801.has(m.slug)&&!gunlukIyilestirmeSlugs20260802.has(m.slug)&&!superseded20260807(m.slug)).map(baseDefaults),
  ...gunlukIyilestirmeler20260801.filter(m=>!retiredRouteSlugs20260803.has(m.slug)&&!gunlukIyilestirmeSlugs20260802.has(m.slug)&&!superseded20260807(m.slug)).map(baseDefaults),
  ...
  ...national.filter(m=>!retiredRouteSlugs20260803.has(m.slug)&&!overridden(m.slug)&&!superseded20260807(m.slug)),
  ...gunlukKaliteIyilestirmeleri20260807.map(baseDefaults),
  ...gun2Meralar20260807.map(baseDefaults),
];

const duplicateSlugs=[...new Set(meralar.map(m=>m.slug).filter((slug,index,items)=>items.indexOf(slug)!==index))];
if(duplicateSlugs.length)throw new Error(`Tekrarlanan avlak slugları: ${duplicateSlugs.join(", ")}`);
